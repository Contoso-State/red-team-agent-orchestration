...
2. **Dispatch the engine.** `node tools/checks/run-checks.mjs --predicates checks/container/predicates.json --rows rows.json --out engagements/<session>/runs/aks-container`. The 13 mechanized control-plane / registry / Pod-Security checks become schema-valid candidates plus a compact `check-summary/v1`. **Schema note:** the deterministic engine predicates are still homed under `compute-platform`, so the engine writes its summary as `compute-platform.json`; but `finding.schema.json`'s `agent` enum now **includes** `aks-container`, so your agent-authored findings (step 7) use `agent: aks-container`. The `AZ-CNTR-` id prefix and the dedicated `--out` keep the engine summary from clobbering the Compute agent's.
...
5. **Runtime probe (C3)** only for a specific, approved workload, with `runtime_probe_per_workload_approval` honored. Attach an **ephemeral debug container** (or a benign read-only `kubectl exec` of inventory commands) — always pass an explicit `--context <in-scope cluster>` on reach-in verbs (`exec`/`debug`/`cp`/etc.); the cluster guardrail denies them otherwise, because the kubeconfig current-context is not trusted for scope-lock. Capture inventory, then remove the ephemeral container. Read-only — never write to the pod, never alter the workload.
