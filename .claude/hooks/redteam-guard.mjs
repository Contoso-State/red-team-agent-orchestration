...
import { EXEC_TOOL } from '../../guardrails/core/guardrails-core.mjs';
...
function extractCommand(input, toolName) {
  for (const args of [input.tool_input, input.toolArgs, input]) {
    if (typeof args === 'string') return args;
    if (!args || typeof args !== 'object') continue;
    // Preserve the shared core's explicit fields and precedence. Generic `input`
    // is a command only for execution tools, never for file content.
    const direct = args.command ?? args.script ?? args.cmd;
    if (typeof direct === 'string') return direct;
    if (EXEC_TOOL.test(String(toolName)) && typeof args.input === 'string') return args.input;
  }
  return null;
}
...
async function main() {
  ...
  const command = extractCommand(input, toolName);
  const cwd =
    input.cwd ||
  ...
  if (typeof command !== 'string') {
    ...
    if (EXEC_TOOL.test(String(toolName))) {
      emit(
        denyOutput(
