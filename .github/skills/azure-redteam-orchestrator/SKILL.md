...
4. **Dispatch preflight (`preflight_inventory`).** Always run `azure-redteam-inventory` first. No
   domain skill runs until the inventory exists and permissions are validated.
   Then execute `build_security_context` to normalize the inventory and any available Defender,
   Entra, Sentinel, ARM, exposure, behavior, and threat-intelligence summaries into
   `state.security_context`. Missing sources remain explicitly unavailable; no signal is inferred.
5. **Fan out the specialists (`plan_specialists` → `run_specialist`).** Based on
   `scope.domains`, `scope.resource_types`, and the inventory, dispatch only relevant domain
   skills **in parallel**, each in its own context, backed by the durable task manifest so the
   fan-out is resumable. Each specialist runs read-only checks, applies a bounded **Self-Refine**
   pass on its own draft, and writes structured findings to
   `engagements/<session>/findings/raw/<agent>.jsonl` per `schemas/finding.schema.json`.
   Pass the shared security context to each specialist and require evidence references back to
   the source summary rather than raw telemetry.
